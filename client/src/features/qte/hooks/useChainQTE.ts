...
export function useChainQTE({
  ...
  useEffect(() => {
    ...
    colyseusService.on("arena:chain_qte:start", handleChainStart);
    colyseusService.on("arena:chain_qte:step_resolved", handleStepResolved);
    colyseusService.on("arena:chain_qte:complete", handleChainComplete);
    colyseusService.on("arena:chain_qte:broken", handleChainBroken);

    return () => {
      clearAllTimeouts();
      colyseusService.off("arena:chain_qte:start", handleChainStart);
      colyseusService.off("arena:chain_qte:step_resolved", handleStepResolved);
      colyseusService.off("arena:chain_qte:complete", handleChainComplete);
      colyseusService.off("arena:chain_qte:broken", handleChainBroken);
    };
  ...
  const respondToStep = useCallback(
    (input: QTEInput, hitPosition: number) => {
      ...
      colyseusService.sendToArena("chain_qte:step_response", response);
    },
