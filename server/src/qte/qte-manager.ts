...
export class QTEManager {
  ...
  initiateAttack(
    ...
    // Enviar QTE para o atacante
    this.sendToClientFn(attacker.ownerId, "qte:start", attackQTE);

    // Broadcast que QTE iniciou (para outros verem)
    this.broadcastFn("qte:initiated", {
      qteId: attackQTE.qteId,
  ...
  private processAttackResponse(
    ...
    // Enviar QTE de defesa para o defensor
    this.sendToClientFn(pending.targetUnit.ownerId, "qte:start", defenseQTE);

    // Broadcast que fase de defesa iniciou
    this.broadcastFn("qte:initiated", {
      qteId: defenseQTE.qteId,
      attackerId: pending.attackerUnit.id,
      targetId: pending.targetUnit.id,
      phase: "DEFENSE",
      attackResult: attackResult.grade,
    });
  }
  ...
  private initiateCascade(
    ...
    // Enviar QTE para o novo defensor
    this.sendToClientFn(newTarget.ownerId, "qte:start", cascadeQTE);
  }
